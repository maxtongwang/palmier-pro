...
- **Upstream PRs are technically blocked, not just discouraged**: `gh` default repo is pinned to the fork, `origin`'s push URL is disabled (fetch-only for syncs), and a PreToolUse hook (`.claude/hooks/block-upstream-pr.sh`) hard-blocks any `gh pr create`/reopen targeting palmier-io — and any bare `gh pr` command that doesn't name `maxtongwang/` explicitly. The global HARNESS auto-PR workflow does NOT apply in this repo.
