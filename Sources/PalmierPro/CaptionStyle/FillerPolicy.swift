...
struct FillerPolicy: Sendable {
    ...
    func strippingRemoveAlways(_ phrase: CaptionBuilder.Phrase) -> CaptionBuilder.Phrase? {
        if !phrase.words.isEmpty {
            ...
            let text = CaptionText.join(kept.map(\.text))
        ...
        // keep Latin whitespace runs whole, and rejoin CJK-aware.
        ...
        return CaptionBuilder.Phrase(text: CaptionText.join(kept), start: phrase.start, end: phrase.end)
    }
    ...
    static func fallbackTokens(_ text: String) -> [String] {
        ...
        for ch in text {
            ...
            } else if ch.unicodeScalars.allSatisfy({ scalar in
    ...
    private func markFillerSpans(
        ...
        // Entries match in BOTH tokenizations, so a multi-character CJK entry (嗯嗯) strips on
        // blob streams and on flattened per-character streams alike.
