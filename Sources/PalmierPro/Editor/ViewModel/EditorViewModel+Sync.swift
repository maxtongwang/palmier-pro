...
extension EditorViewModel {
    ...
    enum SyncDefaults {
        ...
        /// Whole-file alignment (multicam members): devices can start minutes apart.
    ...
    func syncClips(
        ...
        // Seed with capture dates; fall back to audio if needed. Use timecode only if audio fails.
        ...
        for targetId in targets {
            ...
            if let (refCarrier, refTC) = refTCCarrier, let (carrier, targetTC) = tcCarrier(in: unitClips) {
                ...
                // Only trust timecode in explicit timecode mode; auto uses it as a fallback.
                if mode == .timecode {
    ...
    private func envelope(of clip: Clip, fps: Double) async -> AudioEnvelope? {
        let clip = audioBearer(for: clip)
        guard let url = mediaResolver.resolveURL(for: clip.mediaRef) else { return nil }
        let rawStart = Double(clip.trimStartFrame) / fps
        let start = max(0, rawStart)
        let end = rawStart + Double(clip.durationFrames) * max(clip.speed, SyncDefaults.minSpeed) / fps
        guard let env = try? await AudioEnvelopeExtractor.extract(from: url, range: start...max(start + AudioEnvelopeExtractor.hopSeconds, end)) else { return nil }
        guard rawStart < 0 else { return env }
        // A negative trim (multicam bearer) has a silent head — pad so sample 0 stays the clip's start.
        let lead = [Float](repeating: 0, count: Int((-rawStart / env.hopSeconds).rounded()))
        return AudioEnvelope(hopSeconds: env.hopSeconds, samples: lead + env.samples)
    }
}
