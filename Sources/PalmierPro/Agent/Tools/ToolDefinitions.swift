...
enum ToolDefinitions {
    static let all: [AgentTool] = [
        ...
        AgentTool(
            ...
            description: "Change the project's frame rate, resolution, aspect ratio, transcription preference, or on-device model. Pass any combination of fps, explicit width+height, aspectRatio, quality, transcriptionPreference, and transcriptionLocalModel. aspectRatio and explicit width/height are mutually exclusive; quality scales the current aspect ratio (or the selected preset when combined with aspectRatio). The timeline's existing clips are re-fitted automatically: auto-fit transforms recalculate for the new canvas size, and all frame positions/durations rescale when fps changes. transcriptionPreference persists in the project and controls how add_captions/get_transcript/remove_words route: 'auto' (cloud when the signed-in account can afford it, else local — today's default), 'cloud' (force the higher-accuracy cloud model; the call fails with an actionable error when the account can't reach cloud, instead of silently using local), or 'local' (always on-device). transcriptionLocalModel overrides which on-device engine local transcription uses for THIS project (independent of the app-global default), and the response reports resolvedLocalModel. Switching it re-transcribes into a separate cache slot. Undoable (transcriptionPreference/transcriptionLocalModel persist immediately).",
            inputSchema: objectSchema(
                properties: [
                    ...
                    "transcriptionPreference": ["type": "string", "enum": TranscriptionPreference.allCases.map(\.rawValue), "description": "How transcription routes for this project. 'auto' (default): cloud when the account can afford it, else local. 'cloud': force cloud; fail loudly if unavailable rather than degrade to local — use when caption accuracy matters. 'local': always on-device."],
                    "transcriptionLocalModel": ["type": "string", "enum": LocalSpeechEngine.allCases.map(\.rawValue) + ["default"], "description": "Per-project override of the on-device transcription engine, independent of the app-global default. 'qwen3': highest accuracy, native code-switching / 20+ Chinese dialects (qwen3-asr-0.6B-int8, ~840 MB model + a shared ~1.5 GB Whisper timing pass = ~2.3 GB first-run download, ~2 GB RAM in use; the smallest quantization — near-sound errors possible on dense Mandarin). 'whisper': the accurate local fallback when qwen3 mishears — WhisperKit large-v3 turbo (~1.5 GB download, ~1.5 GB RAM), ~100 languages with word-level timestamps. 'apple': system Speech, one language per file from macOS settings, no extra download. 'default' clears the override so the project follows the app-global engine. No larger qwen3 build is published, so 'whisper' is the higher-capacity local option. Does not affect cloud routing (see transcriptionPreference)."],
                ]
            )
        ),
        AgentTool(
        ...
        AgentTool(
            name: .addCaptions,
            description: "Transcribes the timeline's spoken audio and creates styled caption text clips on their own track — no targeting needed; it finds the spoken content itself. Routing follows the project's transcriptionPreference (set via set_project_settings): 'auto' uses cloud when the signed-in account can afford the uncached request and otherwise local, 'cloud' forces cloud (fails with an actionable error if unavailable instead of degrading), 'local' stays on-device. The response reports transcriptionSource and the exact transcriptionModel, plus a low-accuracy note when auto degraded to local. Cloud auto-detects language. Captions are static unless you pass `animation`; when set, per-word animations are timed from the transcript. Style, transform, and maxWords omitted here fall back to the resolved caption_style profile (typography defaults + position); any param you pass wins. fillerPolicy:'removeAlways' drops only removeAlways-classified filler tokens from the caption TEXT (display only — audio is never cut); caseByCase tokens are never auto-removed. Returns the caption group summary (captionGroupId, clipCount, frameRange, shared style, textPreview) — restyle it later with update_text and that captionGroupId.",
            inputSchema: objectSchema(
                properties: mergedProperties([
        ...
        AgentTool(
            ...
            inputSchema: objectSchema(
                properties: [
                    ...
                    "onManualEdits": ["type": "string", "enum": ["preserve", "overwrite", "flag"], "description": "How to treat hand-edited captions. Defaults to the project's conflict policy (flag)."],
        ...
        AgentTool(
            ...
            inputSchema: objectSchema(
                properties: [
                    ...
                    "afterClipId": ["type": "string", "description": "Optional paging cursor. Pass the previous response's nextClipId to lint the next page of caption windows (skips everything up to and including that clip)."],
                ]
            )
        ),
        AgentTool(
