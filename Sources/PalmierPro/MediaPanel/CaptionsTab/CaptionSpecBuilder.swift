...
enum CaptionSpecBuilder {
    ...
    struct Input: Sendable {
        ...
        let segmentation: CaptionBuilder.Segmentation
        let punctuation: CaptionText.PunctuationPolicy
        /// Resolved on the main actor before the build — glossary and style lookups touch disk.
        let protectedPhrases: [String]
        let fillerPolicy: FillerPolicy?
    }
    ...
    static func build(_ input: Input) async throws -> [EditorViewModel.TextClipSpec] {
        ...
        for target in input.targets {
            ...
            let filtered = input.fillerPolicy.map { policy in cased.compactMap { policy.strippingRemoveAlways($0) } } ?? cased
